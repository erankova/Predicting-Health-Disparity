...
class MetricsCV():
    ...
    def objective(self,space,scoring, n_calls=50, n_initial_points=10, verbose=None, pipe=None,
        ...
        val_score_dict = {'Model Name': self.name,
                          'Val Train RMSE': -best_train_score,
        ...
        if val_df is not None:
            new_val_df = pd.concat([val_df, new_val_df])
            new_val_df.index = range(len(new_val_df))
            new_val_df.sort_values(by='Val Test RMSE', inplace=True)
    ...
    def test_metrics(self,X_test,y_test,test_pipe=None,X_train=None, y_train=None,test_df=None):
        ...
        rmse_train = root_mean_squared_error(y_train_full, test_model.predict(X_train_full))
        rmse_test = root_mean_squared_error(self.y_test, test_model.predict(self.X_test))
        ...
        if test_df is not None:
            ...
            new_test_df.sort_values(by='Test RMSE', inplace=True)
